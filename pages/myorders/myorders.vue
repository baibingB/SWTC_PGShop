						...
						<view class="grace-order grace-box-shadow" v-for="(order, orderIndex) in orders[index]" :key="orderIndex">
							...
							<block>
								<view class="grace-title">
									<text class="title grace-black">PG 官方旗舰店</text>
								</view>
								<!-- 循环订单商品 -->
								<view class="grace-order-goods" v-for="(goods, indexGoods) in order.items" :key="goods.goods_id">
									...
									<view class="grace-order-goods-price">PG {{goods.goods_price}}<text class="grace-order-goods-num"> x {{goods.goods_buynum}}</text></view>
								</view>
							</block>
							<!-- 订单底部 -->
...
export default {
	...
	methods:{
		...
		getOrders : function (){
			...
			// page 代表第几页
			// 接口地址组合
			request.get(_self.webUrlUser + '?method=orderlist',{
				fuserid:fuserid,
				ftype:_self.tabs[this.swiperCurrentIndex],
				fpage:_self.pages[this.swiperCurrentIndex]
			}, res => {
				if(res.status == 'ok'){
					// 第一页
					console.log(res);
